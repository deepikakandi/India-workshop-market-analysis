...
## Day 5 Batch 01 outcome

The first real manual batch contains 12 records from Baking Hub, Mud Effects, Kalasaara, SCAI, T-Works, Crust & Crumble, and Mystic Palette. The batch uses `data/manual/observations/2026-08-05/pilot_event_records_batch_01.csv` and preserves collection notes in `data/manual/observations/2026-08-05/batch_01_collection_notes.md`.

Validation passed with zero invalid records, zero governance failures, and no duplicate candidates using explicit `--as-of-date 2026-08-05`. Known shortfalls include missing exact dates for recurring products, missing prices for sources that did not publish prices or displayed ambiguous price text, and limited structured detail on Crust & Crumble's main workshop promotion. Controlled expansion was not performed because the first 12 records passed the initial gate and quality/uniqueness took priority over count.
